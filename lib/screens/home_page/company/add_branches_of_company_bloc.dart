...
class AddBranchesOfCompanyBloc {
  ...
  Future<void> confirmAddBranchesOfCompany(
      {dynamic lat, dynamic lng, BuildContext? context}) async {
    print('signup111');
    ...
    if (validateEmailOrPhone(emailOrPhoneController) &&
      ...
      if (workTimeSubjectFrom.value == null ||
      ...
      if (pdfController.value != null) {
        formData.files.add(MapEntry('pdf',
            PackgDio.MultipartFile.fromFileSync(pdfController.value.path)));
      }
      if (imagesOfCompanyController.value != null) {
        final List<dynamic> srcImages = [];
        for (var sub in imagesOfCompanyController.value) {
          var arr = {'source': PackgDio.MultipartFile.fromFileSync(sub.path)};
          srcImages.add(arr);
        }
        print('oo${srcImages}');
        formData.fields.add(MapEntry('slider', '$srcImages'));
      }
