...
class AddAddressScreen extends StatefulWidget {
  ...
  final int way_pay_id;
   ...
   AddAddressScreen({Key key, this.lat,this.lng,this.company_id, this.pay_method_id, this.way_pay_id, this.paymentPlans}) : super(key: key);
...
class _AddAddressScreenState extends State<AddAddressScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar:  GradientAppbar(title:'text_add_address'.tr,),
                                        ...
                                        child: DropdownButton<CountriesData>(
                                            ...
                                            items: countriesSnapshot.data
                                              ...
                                              return DropdownMenuItem<
                                                  ...
                                                  child: Row(
                                                    children: [
                                                      if (item.icon != null) Image.network(
                                                        '$ImgUrl${item.icon}',
                                                        width: 32,
                                                        height: 32,
                                                      ) else SizedBox(),
                                            ...
                                            onChanged: (CountriesData item) {
                                              _addAddressBloc.selectedCities.sink.add(null);
                                              _addAddressBloc.selectedCountry.sink
                                                  .add(item);
                                            }),
                                      );
                                    });
                            ...
                            builder: (context, snapshot) {
                              if (snapshot.hasData) {
                                return StreamBuilder<CitiesData>(
                                    stream: _addAddressBloc.selectedCities.stream,
                                      ...
                                      return Padding(
                                        ...
                                        child: DropdownButton<CitiesData>(
                                            ...
                                            onChanged: (CitiesData item) {
                                              _addAddressBloc.selectedCities.sink.add(item);
                                             }),
                                      );
                                    });
                              } else
                  ...
                  builder: (context, snapshot) {
                    return TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintStyle: const TextStyle(
                            fontSize: 14, color: Color(0xFF9797AD)),
                        errorText: snapshot.data ? null : 'text_special_mark_error'.tr,
                      ),
                      textInputAction: TextInputAction.done,
                      onSubmitted: (_) => node.unfocus(),
                      keyboardType: TextInputType.text,
                      onChanged: (val) => _addAddressBloc.changeSpecialMark(val),
                ...
                child: RoundedLoadingButton(
                  ...
                  onPressed: () async {
                    _addAddressBloc.loadingButtonController.start();
                     await _addAddressBloc.confirmAddAddress(lat: widget.lat,lng: widget.lng,
                         way_pay_id: widget.way_pay_id,
                     pay_method_id: widget.pay_method_id,company_id: widget.company_id,context: context,paymentPlans: widget.paymentPlans);
