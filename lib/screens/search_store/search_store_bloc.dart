...
class SearchStoreBloc {
  ...
  Future<void> fetchAllSearchStore() async {
    ...
    try {
      ...
      if (res.statusCode == 200 && res.data['status']==200) {
        dataofSearchStoreSubject.sink.add(SearchStore.fromJson(res.data['data']));
        dataListSearchStoreSubject.sink.add(SearchStore.fromJson(res.data['data']).data);
  ...
  Future<void> searchStoreDestroy(int id,BuildContext context) async {
    try {
      ...
      Get.back();
      if (res.statusCode == 200 && res.data['status'] == 200) {
        dataListSearchStoreSubject.sink.add(
            dataListSearchStoreSubject.value..removeWhere((ele) => ele.id == id));
